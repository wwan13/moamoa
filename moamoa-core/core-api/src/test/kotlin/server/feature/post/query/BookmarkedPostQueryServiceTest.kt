...
class BookmarkedPostQueryServiceTest : UnitTest() {
    ...
    fun `캐시된 게시글이 있으면 캐시 메모리에서 결과를 가져와 결과를 병합한다`() = runTest {
        ...
        mockCount(databaseClient, memberId = memberId, totalCount = 2L)
        coEvery { bookmarkedPostListCache.get(memberId, 1L) } returns basePosts
    ...
    fun `캐시된 게시글이 없으면 데이터베이스에서 결과를 가져와 결과를 병합한다`() = runTest {
        ...
        result.posts[1].bookmarkCount shouldBe 4L
        result.posts.all { it.isBookmarked } shouldBe true
        coVerify(exactly = 1) { bookmarkedPostListCache.set(memberId, 1L, basePosts) }
    }
