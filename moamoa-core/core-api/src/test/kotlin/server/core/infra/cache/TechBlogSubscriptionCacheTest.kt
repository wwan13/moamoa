...
import server.core.fixture.createTechBlogSubscriptionInfo
...
class TechBlogSubscriptionCacheTest : UnitTest() {
    ...
    fun `버전이 없으면 기본 버전으로 구독 정보를 조회한다`() = runTest {
        ...
        coEvery { cacheMemory.get(valueKey, any<TypeReference<List<TechBlogSubscriptionInfo>>>()) } returns expected

        val result = cache.get(memberId)

        result shouldBe expected
        coVerify(exactly = 1) { cacheMemory.get(versionKey, any<TypeReference<Long>>()) }
        coVerify(exactly = 1) { cacheMemory.get(valueKey, any<TypeReference<List<TechBlogSubscriptionInfo>>>()) }
    }
