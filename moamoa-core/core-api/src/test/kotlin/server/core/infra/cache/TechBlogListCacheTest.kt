...
import server.core.infra.cache.TechBlogListCache
import server.cache.CacheMemory
import server.core.fixture.createTechBlogSummary
...
class TechBlogListCacheTest : UnitTest() {
    ...
    fun `기술 블로그 목록을 조회한다`() = runTest {
        ...
        coEvery { cacheMemory.get(key, any<TypeReference<List<TechBlogSummary>>>()) } returns expected

        val result = cache.get()

        result shouldBe expected
        coVerify(exactly = 1) { cacheMemory.get(key, any<TypeReference<List<TechBlogSummary>>>()) }
    }
