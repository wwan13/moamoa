...
const PasswordChangePage = () => {
  ...
  const passwordSizeMessage =
    "8자리 이상 32자리 이하로 구성해 주세요. (공백 제외)"
  ...
  const validatePasswordConfirm = (value) => {
    const err =
      !value || value !== newPassword ? passwordNotMatchErrorMessage : ""
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (message === "비밀번호가 일치하지 않습니다.") {
        ...
        return
      } else if (message === "기존 비밀번호가 일치하지 않습니다.") {
        setOldPasswordError(message)
        return
      } else if (message === "같은 비밀번호는 사용할 수 없습니다.") {
        setExtraPasswordError(message)
        return
      }
  ...
  return (
    <div className={styles.wrap}>
      {changePasswordMutation.isPending && <GlobalSpinner />}
