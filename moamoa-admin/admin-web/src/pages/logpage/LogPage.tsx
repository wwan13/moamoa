...
const LOOKBACK_MINUTES_MAP: Record<
  Exclude<LogPeriodOptionValue, "custom">,
  number
> = {
  "10m": 10,
  "30m": 30,
...
const LogPage = () => {
  ...
  const [presetRange, setPresetRange] = useState(initialRange)
  ...
  const customRange = useMemo(() => {
    if (logPeriod !== "custom") return null
    ...
    if (start < maxLookbackStart) return null
    if (end > now) return null
    if (start > end) return null

    return {
      fromTimestamp: formatForApi(start),
      toTimestamp: formatForApi(end),
    }
  }, [customEnd, customStart, logPeriod])

  const activeRange =
    logPeriod === "custom" && customRange ? customRange : presetRange
  ...
  const queryConditions = useMemo(
    () => ({
      ...
      fromTimestamp: activeRange.fromTimestamp,
      toTimestamp: activeRange.toTimestamp,
      size: Number(size),
    }),
    [activeRange, logLevel, logType, size, traceId, traceIdMode],
  )
  ...
  const applyPresetRange = (
    ...
    setPresetRange(toPresetRange(option))
    setCustomStart("")
    setCustomEnd("")
  }
        ...
        <div className={styles.filterTop}>
          ...
          <Dropdown
            ...
            onChange={(value) => {
              ...
              if (option === "custom") {
                setCustomStart(
                  toInputValueFromApiTimestamp(activeRange.fromTimestamp),
                )
                setCustomEnd(
                  toInputValueFromApiTimestamp(activeRange.toTimestamp),
                )
                return
              }
          ...
          {logPeriod === "custom" && (
            <div className={styles.customRangeWrap}>
              <DatePicker
                ...
                min={formatForDatetimeInput(getMaxLookbackStart(new Date()))}
