...
- `moamoa-infra:*‑api` 계약과 구현/ starter 분리·확장
...
## Infra/Starter 원칙
- 인프라 계약(인터페이스/공통 타입/어노테이션)은 `moamoa-infra:*‑api`에 둔다.
- 구현체는 대응하는 기능별 구현 모듈(`*-redis`, `*-caffeine`, `*-resilient` 등)에 둔다.
- starter 모듈(`*-starter`)이 자동구성으로 계약+구현을 묶어 제공한다.
- 구현체 클래스는 기본적으로 `internal`로 숨긴다.
- 애플리케이션(`core-api`, `core-batch`, `admin-api`)은 구현 모듈 직접 의존보다 starter 의존을 우선한다.
