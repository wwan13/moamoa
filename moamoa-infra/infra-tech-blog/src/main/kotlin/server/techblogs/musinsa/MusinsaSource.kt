package server.techblogs.musinsa
...
internal class MusinsaSource(
    ...
    private fun toPost(item: Element): TechBlogPost? {
        val rawUrl = item.selectFirst("link")?.text()?.trim().orEmpty()
        val resolvedUrl = requireField(normalizeUrl(rawUrl), "url", null)
        ...
        val contentHtml = readContentHtml(item)
        val description = readDescription(item, contentHtml)
        val thumbnail = readThumbnail(item, contentHtml)
    ...
    private fun readContentHtml(item: Element): String {
        val element = item.selectFirst("content|encoded")
            ?: item.selectFirst("content\\:encoded")
        return element?.text()?.trim().orEmpty()
    }

    private fun readDescription(item: Element, contentHtml: String): String {
    ...
    private fun readThumbnail(item: Element, contentHtml: String): String {
        val media = item.selectFirst("media|content, media\\:content, media|thumbnail, media\\:thumbnail, enclosure")
            ?.attr("url")
            ?.trim()
            .orEmpty()
        ...
        val fromContent = Jsoup.parse(contentHtml)
            .selectFirst("img")
            ?.attr("src")
            ?.trim()
            .orEmpty()

        return fromContent.ifBlank { DEFAULT_THUMBNAIL }
    }
    ...
    companion object {
        private const val BLOG_KEY = "musinsa"
        private const val FEED_URL = "https://medium.com/feed/musinsa-tech"
